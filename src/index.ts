...
interface Env {
  ...
  FLORALITE_BAIDU_TOKEN_KV: KVNamespace;
}
...
async function ensureToken(env: Env, minTtlMs: number): Promise<StoredToken> {
  ...
  const stored = await env.FLORALITE_BAIDU_TOKEN_KV.get<StoredToken>(TOKEN_KEY, "json");
...
async function refreshToken(env: Env): Promise<StoredToken> {
  ...
  memoryToken = token;
  await env.FLORALITE_BAIDU_TOKEN_KV.put(TOKEN_KEY, JSON.stringify(token));
  return token;
}
